...
\begin{abstract}
Autonomous LLM agents are increasingly being deployed in high-stakes domains such as health triage, financial advice, and public deliberation, where reliability depends not only on what they can do, but on how they decide when to act, wait, defer, or escalate. Existing architectures delegate this activation problem to external orchestrators --- graphs, queues, or heartbeats --- rather than to an endogenous regulatory process. We introduce the \textbf{Pro-Action operator} $\Gamma$, a six-subsystem coupled thermostat used as a \textit{regulatory harness} for LLM policy execution: $\Gamma$ computes an inspectable internal regulatory state, and \textit{Regulatory State Verbalized Interoception} (RSVI) injects that state into the prompt as numerical thermostat values, set-points, and qualitative anchors without directly prescribing the action. In a 920-cell iterated prisoner's dilemma benchmark across three providers, Full-$\Gamma$ exhibits opponent-differentiated cooperation, especially against Grim versus reciprocal opponents. These results support the feasibility of coupling LLM policy execution to explicit regulatory state; stronger claims about architectural necessity, lexical priming, payoff improvement, and generalization beyond this task require additional matched controls and future evaluation.
\end{abstract}
...
Before defining $\Gamma$, we make explicit the modeling commitments that the operator assumes. A1--A3, introduced as the Axioms of Homeostatic Autonomy in a companion work currently under review, are not empirical results or theorems; they state, in this model, what counts as an autonomous, self-regulating agent and what makes an action regulatory rather than merely reactive. This is meant to avoid leaving the agent's ontology implicit in implementation choices. The axiom set was subjected to a formal coherence check using SMT-based tools (Z3) to verify that no pair of axioms is mutually contradictory under the intended semantics; all checks passed.

\textbf{A1 (Autonomy).} An agent is autonomous, in the relevant sense, when its policy depends on an internal state rather than only on external routing: $\pi_i = \pi_i(\delta_i(t))$ for some regulatory deficit $\delta_i$.

\textbf{A2 (Impulse).} The deficit creates activation pressure. A monotone drive $D(\delta_i)$ is mapped through a sigmoid, $p_i(t)=\sigma(D(\delta_i(t)))$, so regulatory imbalance changes the probability or pressure of acting. In IPD, an action is required each round, so this pressure is expressed as modulation of the required discrete choice rather than as the option to remain silent.

\textbf{A3 (Quality gate).} An action is adaptive insofar as it can satisfy the drive that produced it. Formally, a non-negative action-quality signal $g$ measures the effective reduction of the relevant deficit, with $\Delta\delta_i=-\alpha g$ and $\alpha>0$.
...
\end{equation}
with $\odot$ elementwise, $\boldsymbol{\delta}_{\text{pert}}(10) = [0,0,0.5,0,0,0]^\top$ and zero otherwise, and $g_t \in [0,1]$ a normalized action-quality scalar computed from the opponent's last move. The perturbation is a transient load inside the deficit term, not an additive positive shock to the state variable. Values are clipped to $[0,1]$ after each update. The coupling is linear ($W \in \mathbb{R}^{6\times 6}$, sparse, hand-designed; exact values in Appendix~\ref{app:parameters}) with no activation nonlinearity.\footnote{$W$ was finalized in the initial commit (4aacfa7) before any benchmark results were observed; it was never modified post-hoc.} Biological terminology names the functional inspiration; no quantitative biological correspondence is claimed. The present empirical contribution tests structured linear coupling and prompt-mediated regulatory state injection, not implemented delay separation or fast/slow arbitration.
...
\textbf{Smoke-test results (all pass):} (1) Set-point convergence: $\|\mathbf{x}_t - \mathbf{x}^{*}\| < 0.15$ after 10 rounds. (2) Perturbation response remains bounded under the implemented update. (3) 20 iterations without divergence. (4) Driveplexity is an invariant subspace of $\Gamma$ ($\delta = -0.0513$).
...
\begin{table}[t]
\caption{Main 920-cell benchmark. LLM conditions $n=120$; numerical reductions (HRRL, Drive-only) $n=40$. Means with 95\% bootstrap CIs (10k resamples). Range is computed per provider/seed block as max$-$min cooperation across the four opponents, then averaged.}
\label{tab:snapshot}
\centering
\small
...
This paper proposes a self-regulation model for LLM agents. The primary intended impact is scientific: making agent activation auditable and inspectable through explicit thermostat traces. A self-regulation substrate carries dual-use implications. On the positive side, interpretable regulatory state enables auditing of agent behavior and provides a computational substrate for empathy modeling with potential applications in mental health and tutoring. On the risk side, regulatory-state awareness could enable modeling a counterpart's arousal and deliberative load, which may facilitate context-sensitive persuasion. Mitigation: $\Gamma$'s state is monitorable; deployment should be paired with interpretability audits on thermostat traces.
...
\onecolumn
\section{Parameter table and simulation trace}\label{app:parameters}

\begin{table}[t]
\caption{Frozen parameter provenance for the IPD instantiation. Index mapping follows the executed prompt: 0=Attention, 1=Perception, 2=Hormonal/arousal, 3=Emotional valence, 4=Neuropsychological readiness, 5=Cognitive deliberation.}
\label{tab:params}
\centering
\footnotesize
\resizebox{\columnwidth}{!}{%
...
$W$ & see Eq.~\eqref{eq:w-matrix} & functional constraint and wiring provenance \\
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig1_thermostats.png}
