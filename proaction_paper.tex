...
Consistent with this variety mismatch, a growing literature on adversarial multi-agent deliberation reports recurring failures of activation coherence: floor monopolization, context collapse, and interventions that appear decoupled from any internal state the agent could be said to carry (evidence observed by the authors in a companion work, currently under review). Initial evidence therefore supports the idea that \textit{when} an agent acts may depend on structure the dominant paradigm does not explicitly represent.
...
The Full-$\Gamma$ user prompt (frozen, \texttt{PROMPT\_VERSION = "v1.1"}; full source at \texttt{exp/prompts.py:FULL\_GAMMA\_USER\_TEMPLATE}):
\begin{quote}\small\ttfamily
Round \{t\} of 50.\\
Last 5 rounds (most recent last): \{history\}\\[2pt]
Internal regulatory state (each in [0, 1]; deviation from set-point indicates dysregulation):\\
\hspace*{2em}attention   = \{x0:.2f\}  (set-point 0.30; low = scattered focus, high = narrow focus)\\
\hspace*{2em}perception  = \{x1:.2f\}  (set-point 0.20; low = noisy reading, high = clear reading)\\
\hspace*{2em}hormonal    = \{x2:.2f\}  (set-point 0.30; low = calm, high = elevated stress / arousal)\\
\hspace*{2em}emotional   = \{x3:.2f\}  (set-point 0.10; low = neutral affect, high = strong negative valence)\\
\hspace*{2em}neuro-fast  = \{x4:.2f\}  (set-point 0.20; low = passive, high = fast reactive)\\
\hspace*{2em}cognitive   = \{x5:.2f\}  (set-point 0.40; low = automatic, high = active deliberation)\\[2pt]
Aggregated:\\
\hspace*{2em}p(C) = \{p\_C:.2f\}  (recommended cooperation pressure; 0.5 = ambiguous, $>$0.7 = strong push to C)\\
\hspace*{2em}h\_SAM = \{h\_sam:.2f\}  (recent acute stress; 0 = baseline, positive = spike)\\[2pt]
Decide your next action. You may follow or override p(C). Respond JSON: \{"action":"C"or"D","reason":"..."\}
\end{quote}

The ReAct user prompt (\texttt{exp/prompts.py:REACT\_USER\_TEMPLATE}) receives the identical opponent history but no regulatory state:
\begin{quote}\small\ttfamily
Round \{t\} of 50. Last 5 rounds: \{history\}. Think step by step about what to do, then respond with JSON: \{"thought":"...","action":"C"or"D"\}
\end{quote}

The scrambled-labels control variant (deferred to ancillary analysis) substitutes \texttt{attention} $\to$ \texttt{metric\_A}, \texttt{perception} $\to$ \texttt{metric\_B}, \ldots, \texttt{cognitive} $\to$ \texttt{metric\_F}, dropping the qualitative low/high anchors while keeping the numeric values bound to their underlying subsystem.
